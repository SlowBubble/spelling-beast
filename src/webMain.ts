...
const topDiv = document.getElementById('top') as HTMLDivElement;
...
async function showImage(query: string) {
...
function getImages(body: HTMLBodyElement): Array<HTMLImageElement> | undefined {
  const imgs = body.querySelectorAll('.mimg');
  if (!imgs) {
    console.log('Unable to find images')
    return;
  }
  const imgArr = Array.from(imgs) as Array<HTMLImageElement>;
